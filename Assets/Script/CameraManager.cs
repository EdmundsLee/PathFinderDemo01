...
public class CameraManager : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        zoomStrategy = new OrthoZoomStrategy(cam, startingZoom);
        cam.transform.LookAt(transform.position + Vector3.up * lookatOffest);
    }

    private void OnEnable()
    {
        KeyboardInputManager.MoveInput += UpdateFrameMove;
        KeyboardInputManager.RotateInput += UpdateFrameRotate;
        KeyboardInputManager.ZoomInput += UpdateFrameZoom;
        MouseInputManager.MoveInput += UpdateFrameMove;
        MouseInputManager.RotateInput += UpdateFrameRotate;
        MouseInputManager.ZoomInput += UpdateFrameZoom;
    }
